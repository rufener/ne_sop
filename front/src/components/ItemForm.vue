...
export default {
    ...
    computed: {
        ...
        mailtostring() {
            return `mailto:${this.item.users.map(o => o.email).join(";")}&subject=OP ${encodeURIComponent(this.item.number)} - ${encodeURIComponent(this.item.title)}&body=Bonjour,%0D%0A%0D%0AL'objet politique ${encodeURIComponent(this.item.number)} a été modifié.%0D%0A%0D%0AConsulter les modifications: https://sop.ne.ch/items/${this.item.uuid}`
        },
    },
    watch: {
        modelValue: {
